...
const keyboard = {
  ...
  _toggleLanguage() {
    if (this.properties.language === 'rus') { this.properties.language = 'eng'; } else { this.properties.language = 'rus'; }
...
document.addEventListener('keyup', (event) => {
  const code = event.code.substring(0, 3).toLowerCase();
  let key;
  let tapKey;

  if (code === 'key' || code === 'bra' || code === 'sem' || code === 'quo' || code === 'bac' || code === 'com' || code === 'per') {
    if (keyboard.properties.language == 'eng') {
      if (code === 'com' || code === 'per') { key = event.key.toLowerCase(); } else { key = event.code.replace('Key', '').toLowerCase(); }
    } else if (event.key.toLowerCase() === 'backspace') { key = event.key.toLowerCase(); } else { key = typeOfKey[event.code]; }
